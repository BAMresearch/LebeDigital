...
WHERE {
  ...
  ?Specimen cco:is_made_of ?concrete.
  ?concrete rdf:type cst:Concrete.
  ?concrete cco:prescribed_by ?rezeptICE.

  ?rezeptICE obo:RO_0010001 ?rezeptIB.
  ?rezeptIB cco:has_URI_value ?rezept_uri
}
"""
###############################################################################
...
for result in results["results"]["bindings"]:
    print(result)
    if result['rezept_uri']['value'] not in data:
        data['{}'.format(result['rezept_uri']['value'])] = {}

    data['{}'.format(result['rezept_uri']['value'])]['Experiment{}'.format(i)] = {}
    data['{}'.format(result['rezept_uri']['value'])]['Experiment{}'.format(i)]['height'] = {'value' : result['heightvalue']['value'] , 'unit' : result['heightUnit']['value'].split('/')[-1]}
    data['{}'.format(result['rezept_uri']['value'])]['Experiment{}'.format(i)]['diameter'] = {'value' : result['diametervalue']['value'] , 'unit' : result['diameterUnit']['value'].split('/')[-1]}
    data['{}'.format(result['rezept_uri']['value'])]['Experiment{}'.format(i)]['weight'] = {'value' : result['weightvalue']['value'] , 'unit' : result['weightUnit']['value'].split('/')[-1]}
    data['{}'.format(result['rezept_uri']['value'])]['Experiment{}'.format(i)]['LoadDisplacementData'] = result['loaddisplacementdata']['value']

    i+=1

with open('Experiments_with_recipes.yml', 'w') as f:
    yaml.dump(data, f)
