...
i = 0
Height_list = [100,200,300]
for result in results["results"]["bindings"]:

    for height in Height_list:
        if height -10 <= float(result['heightvalue']['value']) <= height+10:

            if '{}'.format(height) not in data:
                data['{}'.format(height)] = {}



            data['{}'.format(height)]['Experiment{}'.format(i)] = {}
            data['{}'.format(height)]['Experiment{}'.format(i)]['height'] = {'value' : result['heightvalue']['value'] , 'unit' : result['heightUnit']['value'].split('/')[-1]}
            data['{}'.format(height)]['Experiment{}'.format(i)]['diameter'] = {'value' : result['diametervalue']['value'] , 'unit' : result['diameterUnit']['value'].split('/')[-1]}
            data['{}'.format(height)]['Experiment{}'.format(i)]['weight'] = {'value' : result['weightvalue']['value'] , 'unit' : result['weightUnit']['value'].split('/')[-1]}
            data['{}'.format(height)]['Experiment{}'.format(i)]['LoadDisplacementData'] = result['loaddisplacementdata']['value']

    i+=1
print(data)
with open('Experiments_with_same_heights.yml', 'w') as f:
    yaml.dump(data, f)
