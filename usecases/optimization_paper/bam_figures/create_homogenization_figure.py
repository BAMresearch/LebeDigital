...
def create_homogenization_figure(parameter_with_units : dict, fig_path:str = "homogenization_plot.pdf"):
    ...
    parameters['paste_kappa'] = float(parameter_with_units['pastekappa']) * ureg(parameter_with_units['pastekappaunit'])
    ...
    # aggregate data
    parameters['aggregates_E'] = float(parameter_with_units['aggregatesE']) * ureg(parameter_with_units['aggregatesEunit'])
    parameters['aggregates_nu'] = float(parameter_with_units['aggregatesnu']) * ureg(parameter_with_units['aggregatesnuunit'])
    parameters['aggregates_C'] = float(parameter_with_units['aggregatesC']) * ureg(parameter_with_units['aggregatesCunit'])
    parameters['aggregates_kappa'] = float(parameter_with_units['aggregateskappa']) * ureg(parameter_with_units['aggregateskappaunit'])
    parameters['aggregates_rho'] = float(parameter_with_units['aggregatesrho']) * ureg(parameter_with_units['aggregatesrhounit'])
