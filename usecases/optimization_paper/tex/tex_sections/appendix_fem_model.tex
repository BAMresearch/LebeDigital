...
The compressive strength in terms of the degree of hydration can be approximated using an exponential function,
\begin{align}
	\fc(\DOH) = \DOH(\zeit)^{\strengthCExp} \fcInf \label{eq:fcwrtDOH}
\end{align}
This model has two parameter, $\fcInf$, the compressive strength of the parameter at full hydration, $\DOH = 1$ and $\strengthCExp$ the exponent, which is a purely numerical parameter.
...
\begin{align}
	\eMod(\DOH) = 
	\begin{cases}
		\eModInf \frac{\DOH(\zeit)}{\DOHt}\left( \frac{\DOHt-\DOHZero}{1-\DOHZero}\right)^{\stiffExp}   
		& \text{for $\DOH < \DOHt$}\\
		\eModInf \left( \frac{\DOH(\zeit)-\DOHZero}{1-\DOHZero}\right)^{\stiffExp}  
		& \text{for $\DOH \ge \DOHt$}
...
\begin{align}
	\eModInf = \dfrac{\eModTwentyEight}{\left(
	\dfrac{\DOHTwentyEight - \DOHZero}
	  {1- \DOHZero}
	  \right)^{\stiffExp}}.
\end{align}
