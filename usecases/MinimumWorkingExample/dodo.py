...
from lebedigital.calibration.posterior_predictive_three_point_bending import (
    perform_prediction,
    wrapper_three_point_bending,
)
...
defaults_dict = {
    "operator_date": ["TIMESTAMP", "operator_date", "operator_date"],
...
# folder with metadata yaml files
metadata_emodulus_directory = Path(emodul_output_directory, "metadata_yaml_files")
...
metadata_mixture_directory = Path(
    mixture_output_directory, "metadata_yaml_files"
)  # folder with mixture metadata yaml files
...
def task_create_openbis_types():
    ...
    yield {
        ...
        "actions": [
            (
                create_required_sample_types,
                [
                    metadata_mixture_directory,
...
def task_upload_to_openbis():
    ...
    for meta_f in list_metadata_emodul_yaml_files:
        ...
        if mix_file:
            mixture_metadata_file_path = Path(
                mixture_output_directory,
                "metadata_yaml_files",
                str(os.path.splitext(os.path.basename(mix_file))[0]) + ".yaml",
            )

            # raw_data_mixture_directory
            mixture_data_path = Path(
                raw_data_mixture_directory, str(os.path.splitext(os.path.basename(mix_file))[0]) + ".xls"
            )
        else:
...
def task_perform_calibration():
    ...
    if config["mode"] == "cheap":
        ...
        list_exp_name = os.listdir(processed_data_emodulus_directory)
    ...
    for f in list_exp_name:
        ...
        def calibrate_it(
            path_exp_data=processed_data_emodulus_directory, path_calibrated_data=calibrated_data_directory, exp_name=f
        ):
            output = read_exp_data_E_mod(
                path=path_exp_data, exp_name=exp_name + ".csv", length=length, diameter=diameter
            )
            E_samples = estimate_youngs_modulus(
                experimental_data=output,
                calibration_metadata={"E_loc": E_loc, "E_scale": E_scale},
                calibrated_data_path=path_calibrated_data,
                mode=config["mode"],
            )
        ...
        yield {
            ...
            "file_dep": [
                exp_metadata_path,
                Path(processed_data_emodulus_directory, f + ".csv"),
            ],  # the file dependancies, the metadata and exp files.
            "targets": [
                owl_file,
                displ_list,
                force_list,
                calibrated_data_file,
            ],  # the files which are output for each calibration
            "clean": [clean_targets],
        }
...
def task_perform_prediction():
    ...
    for f in list_exp_name:
        ...
        calibrated_data = f
        calibrated_data_path = os.path.join(
            calibrated_data_directory, calibrated_data + "/" + calibrated_data + "_calibrated_samples.csv"
        )
        ...
        def do_prediction(samples):
            # perform prediction
            pos_pred = perform_prediction(
                forward_solver=wrapper_three_point_bending, parameter=samples, mode=config["mode"]
            )
