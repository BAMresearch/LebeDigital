...
def test_computation_GWP_per_part():
    ...
    concrete_gwp = computation_GWP_per_part(
        gwp_concrete, 0 * ureg("kg_CO2_eq/m^3"), width, height, length, n_steel, diameter_steel
    )
    steel_gwp = computation_GWP_per_part(
        0 * ureg("kg_CO2_eq/m^3"), gwp_steel, width, height, length, n_steel, diameter_steel
    )
