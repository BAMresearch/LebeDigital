...
from lebedigital.simulation.simple_cube_for_calibration import get_doh_at_28day, get_E_and_fc_over_time
...
def test_get_E_and_fc_over_time():
    Q_ = ureg.Quantity
    # concrete values
    parameters = fenics_concrete.Parameters()
    parameters["density"] = 2350 * ureg("kg/m^3")  # in kg/m^3 density of concrete
    ...
    time_list = [59 * ureg("min"), 2 * ureg("hours")]

    result = get_E_and_fc_over_time(parameters, time_list)

    # check that values exist for all times
    for t in time_list:
        assert t in result["time"].unique()
        assert not np.isnan(result.loc[result["time"] == t, "E"].values[0].magnitude)
        assert not np.isnan(result.loc[result["time"] == t, "fc"].values[0].magnitude)
