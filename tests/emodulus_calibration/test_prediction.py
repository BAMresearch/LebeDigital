...
def test_prediction():
    ...
    if os.path.exists(os.path.join(os.path.dirname(Path(__file__)),"prediction_data.owl")):
        path_calibrated_data = os.path.join(os.path.dirname(Path(__file__)),
                                            "prediction_data.owl")
    else:
    ...
    assert np.mean(pos_pred) == pytest.approx(123.05, rel=0.05)  # this needs to specified by subject matter experts
